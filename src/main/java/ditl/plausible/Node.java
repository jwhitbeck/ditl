...
public abstract class Node {
    ...
    public void writeMovement(long cur_time, long prev_time, double s, double e,
        ...
        final double d = dist(prev_sample, cur);
        if (d < s) { // node hasn't moved this step
            if (paused)
                last_time = cur_time;
            else { // node was mobile until now, stop its movement and pause it
                final double dt = (prev_time - last_time);
                final double ld = dist(last_ref, prev_sample);
                final double sp = ld / dt;
        ...
        } else if (paused) { // move from last_ref
            tube = new Tube(last_ref, e);
            paused = false;
        } else if (!tube.addToTube(cur)) { // current position breaks current
                                           // tube
            final double ld = dist(last_ref, prev_sample);
            final double dt = (prev_time - last_time);
            final double sp = ld / dt;
