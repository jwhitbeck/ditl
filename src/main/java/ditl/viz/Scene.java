...
public abstract class Scene extends JPanel implements TimeChangeListener, Scaler {
    ...
    public Scene()
    {
        ...
        final MouseAdapter mouseAdapter = new MouseAdapter() {
            ...
            public void mouseWheelMoved(MouseWheelEvent e) {
                final int mx = e.getX();
                final int my = e.getY();
                final double os = scale;
                if (e.getUnitsToScroll() > 0)
                    scaleDown();
                else
                    scaleUp();
                final double f = (1 - scale / os);
                dX += (mx - dX) * f;
                dY += (my - dY) * f;
                rescale();
                repaint();
    ...
    protected void rescale() {
        for (final ScaleListener listener : scaleListeners)
    ...
    public void updateSize(double minX, double minY, double maxX, double maxY) {
        ...
        final double xScale = getWidth() / rW;
        final double yScale = getHeight() / rH;
