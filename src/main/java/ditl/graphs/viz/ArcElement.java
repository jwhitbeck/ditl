...
public class ArcElement implements SceneElement {
    ...
    public void paint(Graphics2D g2) {
        ...
        if (state != State.SYM) {
            final int DX = node2.sX - node1.sX;
            final int DY = node2.sY - node1.sY;
            final double theta = Math.atan2(DY, DX);
            final double d = Math.sqrt(DX * DX + DY * DY);
            final double ox = offset / d * DX;
            final double oy = offset / d * DY;
            if (state == State.LEFT)
    ...
    private Path2D.Float createArrowHead(double x, double y, double theta) {
        ...
        final double X1 = x + barb * Math.cos(theta - alpha);
        final double Y1 = y + barb * Math.sin(theta - alpha);
        final double X2 = x + barb * Math.cos(theta + alpha);
        final double Y2 = y + barb * Math.sin(theta + alpha);
        path.moveTo(X1, Y1);
        path.lineTo(x, y);
        path.lineTo(X2, Y2);
        return path;
